...
#                                               #
#################################################
show_interactive_plots = "Off"
if show_interactive_plots == "On":
...
"""
Zooming lattice: Number of old bonds will double (z times, where z is zoom factor) in new lattice. If all spins were alighned, energy would have been (z*n)**2. But, 1/z are old bonds. Magnetization would also increase as system size increase as it is a extensive state variables.
"""
def zoomLattice(z,state,energy,mag):
    Izxz = np.ones((z,z),dtype= "int")
    return (np.kron(state,Izxz), energy*z-(z-1)*(z*n)**2//z, z*z*mag)
...
#this will show configuration of lattice
def showLattice(lattice):
    n = len(lattice)
    plt.axis('off')
    plt.axis([-0.5, n-0.5, -0.5, n-0.5])
    plt.title(f'Temperature {temp}, Size = {n}x{n}')
    plt.imshow(lattice,cmap=plt.get_cmap('gray'),vmin=-1,vmax=1)
    plt.show()
    plt.pause(0.1)
